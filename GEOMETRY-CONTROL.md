...
| CTL-106 | subway_track_spacing_y | 14 | ft | none | D | PLACEHOLDER. Centre-to-centre distance between the two tracks sharing a bay — again a spacing between tracks, not the gauge within one. Bounded by the 28 ft truss bay, and currently set so the pair sits symmetrically in it. Unlike CTL-105, this one **could** be retired at grade B by a published NYCT standard track spacing if one is shown to apply here, exactly as CTL-017 takes gauge from a standard — but a standard states what is normal, not what was built, so a drawing of this bridge would still be needed for grade A. Retired by SRC-025 or SRC-004. See OQ-010. |
