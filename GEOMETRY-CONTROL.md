...
| CTL-105 | subway_track_bay_inner_offset | 27 | ft | none | D | PLACEHOLDER. Transverse distance from the **bridge's own longitudinal axis** to the centreline of the innermost track on each side. **This is a position, not a dimension — it is not gauge.** Gauge is CTL-017, 4 ft 8.5 in, already sourced at grade B; it fixes how wide a track is, and says nothing about where that track sits on the deck. **Bounded**: SRC-011 places the tracks between the inner truss (CTL-022, 20 ft) and the outer truss (CTL-023, 48 ft). The value is not arbitrary but it is still a guess — 27 and 41 ft with CTL-106 puts 7 ft, 14 ft and 7 ft across the 28 ft bay, i.e. the two tracks centred evenly in it. Real alignments are often deliberately offset within a bay for walkway, egress or contact-rail clearance, so symmetry is an assumption rather than a default. **No standard can retire this**, because it is measured from this bridge's axis: only a drawing of this structure answers it. Retired by SRC-025 or SRC-004. See OQ-010. |
| CTL-106 | subway_track_spacing_y | 14 | ft | none | D | PLACEHOLDER. Centre-to-centre distance between the two tracks sharing a bay — again a spacing between tracks, not the gauge within one. Bounded by the 28 ft truss bay, and currently set so the pair sits symmetrically in it. **Bounded below, and now looking too wide.** SRC-026: the B, D, N and Q services crossing this bridge are B Division, whose car bodies are 10 ft wide, so spacing must exceed 10 ft. That floor is genuinely forced — a car cannot be wider than the division it runs in. SRC-027 then supplies the comparator band: the **Williamsburg Bridge**, the other East River suspension bridge carrying B Division trains, runs its transit tracks at **11 ft centres**, and the 1914 Engineering News survey records the Cambridge subway at 12 ft, against 13-15 ft for mainline railroad. **So 14 ft is above the normal rapid-transit band and is probably too generous.** The value is left unchanged rather than swapped for a better guess: 11 ft would still be grade D, Williamsburg carries two tracks where this bridge carries four in two truss bays, and a 28 ft bay leaves room a designer may well have used. What has changed is that the uncertainty now has a direction. **A published NYCT standard spacing would still not retire this**, and the reason is worth stating because it is easy to get wrong. CTL-017 may take gauge from a standard because gauge is *forced*: the B, D, N and Q trains that cross this bridge run through the rest of the network, and rolling stock physically cannot run on a different gauge, so the standard applies by necessity rather than by convention. Nothing forces track *spacing* in the same way. Two tracks may sit any distance apart provided their clearance envelopes do not overlap, and here the governing constraint is a 28 ft truss bay specific to this bridge — not a network-wide figure. Adopting a modern new-construction standard for a 1909 structure would be a number lifted out of the scope in which it was set, which SRC-018 records as its own most-repeated error. The distinction to hold on to: a forced standard can **bound** this value, as SRC-026 now does, without **fixing** it. Bounded by SRC-026; retired by SRC-025 or SRC-004, and by nothing else. See OQ-010. |
...
| OQ-010 | Exact transverse centerlines of the four subway tracks. | Track placement | **SRC-025** (MTA track charts), or SRC-004 | **Mitigated**: tracks are confirmed to lie in the A-B and C-D truss bays, bounding them between 20 ft and 48 ft from centerline. Note CONF-011: the 1908 design put two of them on the upper deck. **Retired-by corrected at Milestone 10**: this pointed only at SRC-004, the NYC DOT contract drawings, but the tracks belong to NYCT rather than to the bridge owner, so DOT may hold no track geometry at all. SRC-025 is a live MTA request (`R016646-081226`) for the track charts that would answer it directly. **What is and is not still open**: track *gauge* is settled — CTL-017, 4 ft 8.5 in, grade B from SRC-010 — and this question does not reopen it. What remains unsourced is *where the four tracks sit across the deck*: the offset from the bridge axis to each track centreline (CTL-105) and the spacing between the pair in each bay (CTL-106). A gauge standard gives the width of a track; it cannot place one on a structure. SRC-018 reaches the same conclusion independently, recording that **no element of either East River subway bridge is `MEASURED` or `DOCUMENTED`** and that the Manhattan floor system is `ASSUMED` — 'placed by engineering judgement with no source statement locating it at all'. |
...
| OQ-025 | **SRC-001 is cited by eight controls but states only four figures.** The NYC DOT Manhattan Bridge page was directly examined on 2026-08-11 and carries exactly: 5,790 ft abutment to abutment at the lower level, 6,090 ft portal to portal on the upper roadway, 1,470 ft main span, and 3,224 ft per cable. It does **not** state the values of CTL-001 (6,855 ft), CTL-004 (2,920 ft), CTL-007 (322 ft tower height), CTL-008 (135 ft clearance) or CTL-009 (46 ft roadway width), all of which cite it. | Citation accuracy. **No value is known to be wrong**, and CTL-001, CTL-004 and CTL-008 each carry three or more other sources. CTL-007 and CTL-009 are the exposed pair: each rests on SRC-000 plus SRC-001, so if SRC-001 does not carry them they rest on SRC-000 alone. | SRC-004, or a dated capture of SRC-001 | New at Milestone 10, found while preparing FOIL-REQUEST.md. Two innocent explanations exist and this repository cannot presently distinguish them: the page may have listed more figures when SRC-001 was registered, or the citation may have been inherited from SRC-000's handoff table without being checked against the page. Recorded rather than corrected, because removing a citation implies the value is unsupported and that is not established. It matters because CTL-007 feeds CONF-005, where four sources give four different tower heights, and CTL-009 feeds OQ-004, which asks which deck the 46 ft roadway belongs to. |
...
| MAT-007 | `*_main_cable_*` | steel_wire | SRC-011, SRC-015, SRC-006 | B | The sources establish the cable is built of 9472 drawn wires in 37 strands (CTL-041) and give the wire diameter (CTL-043), but **no registered passage names the metal**. Steel is inferred from the structure type and period. See OQ-019. **Promoted C to B at Milestone 11.** Eight frames were accepted under `cable`; `obs-61d3e2f9a9d1` and `obs-be6c5ed93389` show the main cables as **wrapped and painted cylindrical sections with regularly spaced cast cable bands**, which corroborates the wire construction the period sources describe and adds the finish they do not. |
| MAT-008 | `suspenders_*` | steel_wire | SRC-011, SRC-006 | B | As MAT-007. Suspender pitch is derived, not the material. **Promoted C to B at Milestone 11** on the same eight `cable` frames. The suspenders read as **paired wire ropes dropping from cast bands to the truss top chord**, visible in `obs-61d3e2f9a9d1` along the whole span. Consistent with Contract 14 having replaced all 628 suspenders in 2010-2013, which is why this is graded on the photographed condition rather than the original. |
| MAT-009 | `stiffening_truss_*` | steel_structural | SRC-011, SRC-006 | B | Three sources agree the stiffening trusses are Warren trusses 24 ft deep (CTL-010) but none that is registered names the metal. Inferred, consistent with the sourced steel towers. See OQ-019. **Promoted C to B at Milestone 11**, on a human-reviewed photographic corpus rather than on inference. Thirteen frames from SRC-006 were accepted under the `steelwork` category by a named reviewer; `obs-61d3e2f9a9d1` shows the stiffening trusses square-on from the roadway and `obs-dbcf8540c728` shows a tower-line connection close enough to count individual rivet rows. What they establish is fabric, not dimension: **riveted built-up plate and lattice construction in painted structural steel**, with dense field rivets, no welds visible anywhere, and a two-tone pale grey and dark grey paint system. That is a statement about what the members ARE, which CONFIDENCE-MODEL.md section 6.2 permits a photograph to carry. It says nothing about CTL-010's 24 ft depth, which remains sourced from SRC-011 alone. |
...
| MAT-011 | `*_approach*` | steel_structural | SRC-006 | B | PLACEHOLDER. See OQ-020; the approach viaduct's structural form and material are both unregistered. **Promoted D to B at Milestone 11** for MATERIAL ONLY. `obs-5d6e036fed6b`, looking along Adams Street beneath the Brooklyn approach, and `obs-90dddcf6d96c` show the approach structure as **riveted steel plate girders and lattice bracing under dark paint**, continuous with the suspended span's construction. **OQ-020 stays fully open**: this settles what the approach is made of and says nothing about bent spacing, girder depth or grade, which are CTL-108 to CTL-110 and still placeholders. A photograph can settle fabric without settling form. |
...
| MAT-013 | `*_roadway_*` | roadway_surface | SRC-006 | B | PLACEHOLDER. No registered source describes the wearing surface on either level. **Promoted D to B at Milestone 11.** `obs-61d3e2f9a9d1`, taken from a vehicle on the upper roadway, shows the wearing surface unambiguously: **bituminous asphalt with painted white lane markings and a concrete kerb upstand at the truss line**. The reviewer also tagged `obs-2465a90e450e` under `deck`. Note the temporal caveat that the campaign records for this category: a roadway surface is `renewed`, so this describes the surface as photographed and not as built in 1909. Nothing here dates it. |
