...
| OQ-021 | The presentation layer stands in for evidence that does not exist yet, and must be replaced by measured capture. | Rendered appearance only. **No dimension depends on this.** | SRC-004, a walkway image set, SfM with scale control | New at Milestone 8. See CONFIDENCE-MODEL.md section 7. Open items: (a) the tower arch openings visible in SRC-018's HAER photograph are **not modelled** — the towers are plain tapered boxes, so the render understates them; (b) the truss web is drawn as lines rather than members, because OQ-017 leaves the diagonal handedness unknown; (c) no surface carries real texture, only a graded material finish; (d) the ornamental finials on the tower tops are not modelled at all. Retired progressively as each becomes sourced geometry. |
