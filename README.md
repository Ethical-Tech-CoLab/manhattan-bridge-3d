...
*Both come from [`digital-3d-shared-contracts`](https://github.com/Ethical-Tech-CoLab/digital-3d-shared-contracts): the viewer is `@d3d/bridge-viewer-ui`, vendored into `viewer/shared/` and checked against a hash manifest on every build, and the audit sheet is generated by `tools/photo-survey`. This repository owns no viewer code beyond `viewer/src/main.tsx`.*
